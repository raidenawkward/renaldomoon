...
*Windows和UNIX是不同时代, 不同背景下, 不同文化的产物*
...
##何时收集
- 监控程序崩溃, 发现后收集崩溃信息, 完成后立刻发送, 发送完成后程序退出
- 监控程序崩溃, 发现后收集崩溃信息, 完成后保存在本地, 程序退出. 再次启动程序后发送报告
...
- 操作系统和必要软件版本
- 发生崩溃的代码文件名和行号
- 字符串格式的错误信息 (可做为bug的分类标识, 用于去重)
- 错误代码
- 用户对出现路径的描述
- 用户电子邮件(联系方式)
- 系统调用栈/内存dump/core dump不一定有用
...
##处理方式
- 通过"字符串格式的错误信息"做为问题点标识
- 发送报告的API只用于上传信息, 防止被攻击
- 发送报告的API可收到一条用于提示用户的信息("此问题已经被他人报告, 我们正在xxx, 临时解决方案如下xxx")
...
##**当软件足已解决某个商业问题时, 无论怎样提高它的质量, 投资回报率都是0**